...
$status=2;
$cancelby='a';
$sql = "UPDATE chat_group SET status=:status,CancelledBy=:cancelby WHERE Id=:eid";
...
$msg="Group Cancelled successfully";
}
...
$status=1;
$cancelby='a';
$sql = "UPDATE chat_group SET status=:status WHERE Id=:evid";
$query = $dbh->prepare($sql);
$query -> bindParam(':status',$status, PDO::PARAM_STR);
				...
				<div class="agile-tables">
					<div class="w3l-table-info">
					  <h2>Manage ChatGroup</h2>
						...
						<tbody>
<?php 
$sql = "SELECT chat_group.Id as id,tblusers.FullName as fname,tblusers.MobileNumber as mnumber,tblusers.EmailId as 
email, chat_group.GroupName as GroupName, chat_group.Purpose as
 Purpose, chat_group.status as status, chat_group.CancelledBy as cancelby
  from tblusers join chat_group on chat_group.UserEmail=tblusers.EmailId";
$query = $dbh -> prepare($sql);
$query->execute();
...
$sql= "SELECT UserEmail from chat_group";
$query = $dbh->prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
$cnt=1;
